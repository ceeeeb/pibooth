...
class PiConfigMenu(object):
    ...
    def __init__(self, plugins_manager, configuration, application, window, onclose=None):
        ...
        self.size = (int(size[0] * 0.85), int(size[1] * 0.85))
    ...
    def process(self, events):
        ...
        if self.is_confirming():
            self._confirm_menu.update(events)
            if self._confirm_menu.is_enabled():  # callback may have closed it
                self._confirm_menu.draw(self.win.surface)
            return
